...
| **N6** | **Done.** Long-idle stale `HubConnection`: signalr_core 1.1.2's `stop()` never returns when it lands on an in-flight start that then fails, and `SignalRService` awaited it without a bound, so no fresh connection could be built and Reconnect stayed stuck. `stop()` is now bounded by `stopTimeout` in `_cleanupConnection`, `_stopHubForNoInternet` and `disconnect`; callbacks and teardown `finally` blocks ignore a hub that is no longer current. `signalr_service.dart` only. Covered by `test/signalr_stop_timeout_test.dart` (8 tests, all **proven to fail** against the old code). `flutter analyze` **0 issues**; `flutter test` **1983 passing / 0 failing**. Detail in [CHANGELOG.md](CHANGELOG.md). | **DONE** |
| **N7** | Private join-by-code is not retried after the hub recovers. If the hub is not connected at entry, `joinPrivateGame`'s `invoke` returns false and reopens its guard, but `onRecovered()` retries only `JoinRandomGame` and `CreatePrivateGame`, and the code is not stored. **VERIFIED by code reading, not reproduced.** Found during the N6 investigation; deliberately not fixed there. | `OPEN` |
| **N8** | A thrown `invoke` leaves the entry guard claimed. If the hub closes mid-invocation, `invoke` throws, so `_didJoinRandom` / `_didCreatePrivateGame` are never reset and `onRecovered()` skips the retry. **VERIFIED by code reading, not reproduced.** Found during the N6 investigation; deliberately not fixed there. | `OPEN` |
