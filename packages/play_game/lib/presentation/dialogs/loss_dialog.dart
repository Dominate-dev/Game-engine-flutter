...
/// "You lost" dialog — same card/ribbon/stars layout as [WinDialog], but
/// without the confetti Lottie and with the greyscale stars + "good luck"
/// messaging instead of the celebratory win copy.
///
...
class LossDialog extends ConsumerWidget {
  const LossDialog({
    super.key,
    this.winner,
    this.me,
  ...
  /// the caller already falls this back to [opponent] when it cannot be
  /// resolved (no gameOver / no winnerId / neither seated player matches).
  ...
  Widget _cardUi(BuildContext context, PlayGameStrings strings) {
    return Stack(
      ...
      children: [
        ...
        const Positioned(
          top: -46,
          child: AppImageView(
            assetPath: AppAssets.starsLossIcon,
